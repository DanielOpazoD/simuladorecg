...
const fixture=arg('--fixtures'),output=arg('--output');
const sourceRoot=process.argv.includes('--source-root')?arg('--source-root'):process.cwd();
const source=f=>resolve(sourceRoot,f);
...
if(JSON.stringify(Object.keys(built.metafile.inputs).map(f=>relative(sourceRoot,resolve(f))).sort())!==JSON.stringify(Object.keys(protocol.analyzerFiles).sort()))throw new Error('Unexpected analyzer dependency');
